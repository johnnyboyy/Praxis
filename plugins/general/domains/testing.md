...
- id: scaffolding-tests-are-disposable-boundary-tests-survive
  ...
  reason: "An implementation-coupled test is an ATTRACTOR — during a rebuild it pulls the work back toward the current implementation because the test restates the implementation rather than the contract. Keeping only the boundary test leaves the internals genuinely free to change while the contract stays guarded. This is the same move as the rebuild triple (extract the contract as the IR, synthesize freely, coverage-diff against the contract, not the implementation). Relates to interface-is-the-test-surface (codebase-design domain) and unit-test-only-for-named-reasons (this domain)."
